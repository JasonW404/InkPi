...
| **inkpi-admin** | `inkpi-admin` | Local web admin portal on LAN/hotspot, port 8080 (本地 Web 管理门户) | ✅ Running |
...
`inkpi-admin` runs as a third systemd service, binding port 8080 and serving a local management interface over LAN or hotspot.

`inkpi-admin` 在 systemd 部署中作为第三个服务运行，绑定端口 8080，通过 LAN 或热点网络提供本地管理界面。
